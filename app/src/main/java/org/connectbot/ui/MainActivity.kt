...
class MainActivity : AppCompatActivity() {
    ...
    // 注入 TerminalManagerProviderImpl，用于 MCP 功能获取 TerminalManager
    @Inject
    lateinit var terminalManagerProvider: TerminalManagerProviderImpl
    ...
    private val connection = object : ServiceConnection {
        override fun onServiceConnected(name: ComponentName?, service: IBinder?) {
            ...
            terminalManagerProvider.setManager(manager)
            bound = true
        }
        ...
        override fun onServiceDisconnected(name: ComponentName?) {
            ...
            terminalManagerProvider.setManager(null)
            bound = false
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // MCP 服务器若已启用，随应用启动自动恢复前台服务
        val prefs = PreferenceManager.getDefaultSharedPreferences(this)
        if (prefs.getBoolean(McpService.KEY_MCP_ENABLED, false)) {
